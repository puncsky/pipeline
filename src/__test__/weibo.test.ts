...
test.skip("weibo_client_test", async t => {
  const weibo = new WeiboClient(cfg);

  const url = await weibo.send({
    content: "just for test for clinet!"
  });

  t.truthy(url.startsWith("https://weibo.com"));
});
